...
data = randi([0 1], N, 1); %Random data is generated
iterations = 100; %Number of Iterations

%Inputs bits are modulated using BPSK
Ak = 2*data-1;  % 0->-1, 1->+1
...
for i = 1:length(SNR)
    ...
    h_rayleigh = sqrt(1/2)*(randn(N, 1) + 1j*randn(N, 1)); %Fading Channel Coefficient
    Yk_gaussian = Ak + noise;
    Yk_Rayleigh = h_rayleigh.*Ak + noise;
    %% rician
    k1=10; %Rician factor
    ...
    Yk_Rician=Ak.*h_rac+noise; % s means transmitted signal...please take the value as u have taken
    
    %% nakagami -m
    m =2;
    h_naka = sqrt(m)*sqrt(1/2)*(randn(N, 1) + 1i*randn(N, 1));
    %h_naka = transpose(sqrt(m)*sqrt(1/gamrnd(m,1/m,[N,1])));
    YK_nakagami = Ak.*h_naka + noise;
    %% detections
    detected_awgn = real(Yk_gaussian)>0;
    y=Yk_Rayleigh./h_rayleigh;
    detected_rayleigh = real(y)>0;
    z1=Yk_Rician./h_rac;
    detected_rician = real(z1)>0;
    detected_nakagami = real(conj(h_naka).*YK_nakagami)>0;
    %% error
    errors_awgn = sum(detected_awgn ~= data);
    errors_rayleigh = sum(detected_rayleigh ~= data);
    errors_rician = sum(detected_rician ~= data);
    errors_nakagami = sum(detected_nakagami ~= data);
...
xlabel("SNR(in dB)");
ylabel("BER");
title("BER of BPSK in AWGN, Rayleigh, Racian and Nakagami-m Channels")